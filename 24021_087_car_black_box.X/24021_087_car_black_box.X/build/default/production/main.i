...
State_t state;
unsigned char key;
char gears[10][3] = {"ON", "GN", "G1", "G2", "G3", "G4", "G5", "GR", "C_"};
char menu[4][16] = {"View log", "Clear log", "Download log", "Set time"};
char data[10][16];
char menu_index = 0, gear_index = 0;
char flag = 1, flag1 = 0, flagER = 1;
unsigned short adc_reg_val;
unsigned char address = 0x00;
unsigned char no_of_events = 0;
...
void view_log() {
    ...
    clcd_print(data[index], (0xC0 + (2)));
}

void clear_log(void) {
    clcd_print("ALL LOGS        ", (0x80 + (0)));
    clcd_print("CLEARED :-)     ", (0xC0 + (0)));
    if (delay++ == 500) {
        delay = 0;
        address = 0x00;
        no_of_events = 0;
...
void download_log(void)
{
    ...
    else
    {
        ...
        while (index < no_of_events)
        {
            for (int i = 0; i < 10; i++)
            {
                log[i] = read_external_EEPROM(add++);
            }
...
void set_time(void)
{
    ...
    if (once == 0)
    {
        ...
        new_time[1] = (time[3] - '0') * 10 + (time[4] - '0');
        new_time[2] = (time[6] - '0') * 10 + (time[7] - '0');
    }
    ...
    if(blink_delay < 50)
    {
        ...
        clcd_putch(new_time[0] % 10 + '0', (0xC0 + (2)));
        clcd_putch(':', (0xC0 + (3)));

        clcd_putch(new_time[1] / 10 + '0', (0xC0 + (4)));
        clcd_putch(new_time[1] % 10 + '0', (0xC0 + (5)));
        clcd_putch(':', (0xC0 + (6)));

        clcd_putch(new_time[2] / 10 + '0', (0xC0 + (7)));
        clcd_putch(new_time[2] % 10 + '0', (0xC0 + (8)));
        blink_delay++;
    }
    else if(blink_delay < 1000)
    {
        if(blink_pos == 0){
            clcd_print("\xFF\xFF", (0xC0 + (1)));
        }else if(blink_pos == 1){
            clcd_print("\xFF\xFF", (0xC0 + (4)));
        }else if(blink_pos == 2){
            clcd_print("\xFF\xFF", (0xC0 + (7)));
        }
        blink_delay++;
    }
    else
